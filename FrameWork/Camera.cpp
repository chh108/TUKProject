...
void CSpaceShipCamera::Rotate(float x, float y, float z)
{
	if (m_pPlayer && (x != 0.0f))
	{
		...
		m_xmf3Position = Vector3::Subtract(m_xmf3Position, xmf3PlayerPos);
...
CAimPersonCamera::CAimPersonCamera(CCamera* pCamera) : CCamera(pCamera)
{
	m_nMode = THIRD_PERSON_CAMERA;
	if (pCamera)
	{
...
void CAimPersonCamera::Update(const XMFLOAT3& xmf3LookAt, float fTimeElapsed)
{
	if (m_pPlayer)
	{
		XMFLOAT4X4 xmf4x4Rotate = Matrix4x4::Identity();
...
void CAimPersonCamera::SetLookAt(const XMFLOAT3& xmf3LookAt)
{
	XMFLOAT3 adjustedLookAt = xmf3LookAt;				//+
	adjustedLookAt.y += 100.0f; // 타겟 Y축을 100만큼 아래로 설정		
...
void CAimPersonCamera::Rotate(float x, float y, float z)				//함수 +
{
	if (y != 0.0f) // 좌우 회전 (Yaw)
	{
		m_fYaw += y;
